...
- [x] FR-PUB-02 jobs pages (2026-09-03) — Added SSR `/jobs` listing and static `/jobs/[slug]` details from typed seeded sample jobs, category/language query filters, JobPosting JSON-LD, canonical metadata, sitemap/public-route registration, and footer Jobs link. Jobs remains intentionally noindex and excluded from primary navigation until real postings are available. Refined the UI with DESIGN.md token-based gradient/ambient treatment, glass surfaces, elevated card hover states, responsive grids, focus-visible controls, and reduced-motion handling. Complete repository test suite passes: 8/8 tasks, 141/141 web tests.
...
FR-PUB-02 metadata follow-up (2026-09-04): both seeded job routes now declare page-level `robots.index: false`; sitemap and robots disallow behavior remain unchanged. Targeted tests were added; lint passes, but Vitest startup is blocked by an existing workspace filesystem access error.
