...
**Authoritative status (2026-09-04):** FR-PUB-02 is complete. Phase 1 has only FR-SEO-01 deferred. `/jobs` remains intentionally noindex and excluded from primary navigation until real postings are available; it is linked from the footer. The full repository test suite is green.
<!-- Historical status snapshot retained below for context; do not use as current status.

**Status correction (2026-09-03): FR-PUB-02 is complete.** The `/jobs` listing and `/jobs/[slug]` detail pages are SSR/static web routes backed by typed seeded sample jobs, with category/language filters, `JobPosting` JSON-LD, canonical metadata, route-registry and sitemap inclusion, Jobs navigation, and DESIGN.md-aligned responsive glass UI. The full repository test suite is green: 8/8 workspace tasks and 293 tests. FR-SEO-01 remains the only deferred Phase 1 public FR; external Google Rich Results validation remains pending deployment of a public job detail URL.
...
- FR-PUB-02 is complete. Code-side JobPosting JSON-LD, route-registry assertions, sitemap inclusion, and the full test suite pass. Run the external Google Rich Results test after deployment of a public `/jobs/[slug]` URL.
