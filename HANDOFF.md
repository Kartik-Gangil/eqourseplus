...
- FR-PUB-02 is complete. Code-side JobPosting JSON-LD, route-registry assertions, footer linking, intentional noindex metadata, and exclusion from `RESOLVING_ROUTES` are in place; `/jobs` remains out of the sitemap until FR-PRJ supplies real postings. Run the external Google Rich Results test after deployment of a public `/jobs/[slug]` URL.
